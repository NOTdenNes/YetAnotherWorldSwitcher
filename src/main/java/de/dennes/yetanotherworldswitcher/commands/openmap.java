...
public class openmap implements TabExecutor
{
    ...
    public openmap(YetAnotherWorldSwitcher yaws)
    {
        this.yaws = yaws;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args)
    {
            ...
            for(lobby lobby : getLobbyList())
            {
                if(lobby.getName().equalsIgnoreCase(player.getLocation().getWorld().getName())) {
                    ...
                    if (lobby.isClosed())
                    {
                        ...
                        yaws.getLogger().info("World " + lobby.getName() + " got unlocked by " + player.getName());
                    }
                    else
                    {
            ...
            for (lobby lobby : getLobbyList())
            {
                if(lobby.getName().equalsIgnoreCase(args[0]))
                {
                    if(lobby.isClosed())
                    {
                        ...
                        {
                            Player player = (Player) sender;
                            player.sendMessage("World " + ChatColor.GOLD + ChatColor.ITALIC + lobby.getName() + ChatColor.RESET + " is now open to the public.");
                            yaws.getLogger().info("World " +  lobby.getName() + " got unlocked by " + player.getName());
                        }
                        else
                        {
                            yaws.getLogger().info("World " +  lobby.getName() + " is now open to the public.");
                        }
                    }
                    else
                    {
                        ...
                        {
                            Player player = (Player) sender;
                            player.sendMessage("World " + ChatColor.GOLD + ChatColor.ITALIC + lobby.getName() + ChatColor.RESET + " is already open to the public.");
                        }
                        else
                        {
                            yaws.getLogger().info("World " +  lobby.getName() + " is already open to the public.");
                        }
                    }
                    return true;
                }
        ...
        {
            yaws.getLogger().warning("An unknown error occurred whilst trying to open a map. Please contact the developer of this plugin");
            return true;
        }
        return true;
    }
